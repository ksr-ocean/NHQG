"""Checkpoint helpers for the trig benchmark solver."""
...
from trig_vertical_benchmark.solver import State


def save_checkpoint(state: State, step: int, cfg: TrigBenchmarkConfig, output_dir: str | None = None) -> str:
    """Save a trigonometric benchmark checkpoint."""
...
def load_checkpoint(path: str, dtype=jnp.complex128) -> tuple[State, int, float]:
    """Load a trigonometric benchmark checkpoint."""
    data = np.load(path)
